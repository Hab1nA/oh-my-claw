...
- **TUI 界面** — 内置终端聊天界面，支持彩色输出、时间戳显示、内置命令
...
```bash
# 方式一：运行 TUI 终端界面（推荐本地开发，需要 API 密钥）
npm run dev:tui

# 方式二：运行独立演示（无需 API 密钥，验证组件初始化）
npm run demo

# 方式三：运行完整示例（需要 Telegram Bot Token）
npm run dev

# 方式四：运行 Gateway 模式（提供 HTTP/WS 接口）
npm run dev:gateway
```

## �️ TUI 终端界面

TUI（Terminal User Interface）是 OpenClaw 内置的终端聊天界面，提供本地开发和测试的便捷方式。
...
|------|------|
| `/help` | 显示所有可用命令 |
| `/status` | 显示当前会话信息（Session ID、User ID 等） |
| `/clear` | 清屏并重新显示横幅 |
| `/quit` 或 `/exit` | 退出 TUI 会话 |
...
| `showTimestamp` | `boolean` | `false` | 是否在消息中显示时间戳 |
| `greeting` | `string` | OpenClaw 横幅 | 启动时显示的问候消息 |

### 环境变量

运行 TUI 需要配置 LLM API 密钥：
